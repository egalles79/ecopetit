<?php
$FM_VERS = "8.21";      // script version

/* ex:set ts=4 sw=4 et:
 * FormMail PHP script from Tectite.com.  This script requires PHP 4 or later.
 * Copyright (c) 2001-2009 Root Software and Open Concepts (Vic) Pty Ltd
...
$TARGET_EMAIL = array(EMAIL_NAME."@ecopetit\.cat$");
...
$REQUIRE_CAPTCHA = "";          // set to a message string if your forms
            ...
            /* Help: http://www.tectite.com/fmdoc/recaptcha.php */
...
function LoadBuiltinLanguage()
{
    ...
    $aMessages[MSG_RECAPTCHA_MATCH] = 'reCaptcha verification failed';
                        ...
                        $b_show_mnum = true,$b_no_errors = false)
{
    ...
    if (strpos($s_text,'$') !== false)
    {
        ...
        $aGetMessageSubstituteErrors = array();
        $aGetMessageSubstituteFound = array();
        $aGetMessageValues = HTMLEntitiesArray($a_params);
...
register_shutdown_function('OnExit');
    ...
    //
function HTMLEntitiesArray($a_array)
{
    foreach ($a_array as $m_key=>$s_str)
    {
        ...
        if (($i_pos = strpos($s_str,'=')) !== false)
            $a_array[$m_key] = substr($s_str,0,$i_pos+1).
                                FixedHTMLEntities(substr($s_str,$i_pos+1));
        else
            $a_array[$m_key] = FixedHTMLEntities($s_str);
    }
    return ($a_array);
}

    //
    // Unfortunately, htmlentities gets some characters
    // wrong and converts them even when the charset is provided.
    // This function overcomes this problem.
    //
function    FixedHTMLEntities($s_str)
{
    global  $sHTMLCharSet;
...
function URLEncodeArray($a_array)
{
    ...
    //
function    EncodeHeaderText($s_text)
{
    global  $sHTMLCharSet;
    ...
    if ($s_charset != "")
    {
            ...
            // quoted printable is usually a better choice
            //
        //return ("=?".$s_charset."?B?".base64_encode($s_text)."?=");
        return ("=?".$s_charset."?Q?".quoted_printable_encode($s_text)."?=");
    }
    else
        return ($s_text);
}
...
{
        //
        // and email address is made up of local and domain parts
        ...
        // This is probably not necessary because there shouldn't be
        // any control characters at this point, but this is just extra
        // safety.
        //
    $s_str = preg_replace('/[[:cntrl:]]+/',"",$s_str);    // zap all control chars
    return ($s_str);
        ...
        if (($a_lines = $http_get->Read()) === FALSE)
        {
            ...
            default:
                $s_error = GetMessage(MSG_SOCKET,
                                array("ERRNO"=>$i_sys_err,
                                    "ERRSTR"=>$s_sys_msg,
                                    "PHPERR"=>CheckString($php_errormsg)));
                break;
            }
            SendAlert($s_error);
            return (false);
...
{
        //
        // note that [:cntrl:] etc seems to work with preg_replace, even
        // though I can't find documentation for it!
        //
    ...
    function    MakeCSVRecord($a_column_list,$a_vars)
    {
        ...
        for ($ii = 0 ; $ii < $n_columns ; $ii++)
        {
            $s_col_name = $a_column_list[$ii];
                 ...
                 * is no data for it.
                 */
            $m_value = GetFieldValue($s_col_name,$a_vars);
...
function SubstituteValueWorker($a_matches,$s_repl,$b_html = true)
{
    ...
    {
        if (isset($aSubstituteValues[$s_name]) && is_array($aSubstituteValues[$s_name]))
                //
...
function DetectJunkAttack($a_fields,&$s_attack,&$s_info,&$s_user_info)
{
    ...
    foreach ($a_fields as $s_fld=>$m_value)
    {
        ...
        if (in_array($s_fld,$ATTACK_DETECTION_JUNK_IGNORE_FIELDS,true))
            continue;
        if (isset($m_value) && !IsFieldEmpty($m_value))
        {
            if (!is_array($m_value))
        ...
        foreach ($this->_aAccessList as $s_var_name)
            if (IsSetSession($s_var_name))
                if (!isset($a_vars[$s_var_name]) ||
                        ($b_overwrite_empty &&
                        IsFieldEmpty($a_vars[$s_var_name])))
                {
...
ProcessFilterOptions($aAllRawValues);
    ...
    // fields that were assumed to be clean, then an attacked could create
