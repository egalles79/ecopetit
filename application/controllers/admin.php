...
class admin extends controlador {
    ...
    */
    ...
    function articles() {
        ...
        if ($this->input->post('frmArticles')) 
        {
            $dataPost['CodiBarres'] = $this->input->post('codiBarres');
            $dataPost['publicat']   = $this->input->post('publicat');
            $dataPost['idGrup']     = $this->input->post('idGrup');
            $dataPost['idMarca']    = $this->input->post('idMarca');
            $dataPost['model']      = $this->input->post('model');
        ...
        $data['main_template']  = 'users/publicar_articles';
        $data['title'] = lang('user.opcio1'); 
        $data['description'] = "";
        $this->load->model('grupproducte_model');
        $this->load->model('productes_model');
        $this->load->model('marques_model');
        $data['grupsProd'] = $this->grupproducte_model->getData();  
        $data['llistaMarques'] = $this->marques_model->getData();   
        $data['result'] = $this->productes_model->getProductes($dataPost);  
        $data['PublicaArticles'] ="S";
...
function Redimensionar($Ruta, $Imatge, $w, $h) {

    //Se define el maximo ancho o alto que tendra la imagen final
    $max_ancho = $w;
    $max_alto = $h;
        
    $rutaImagenOriginal = $Ruta.$Imatge;
    ...
    function novaEntrada() {
        ...
        $this->load->model('blog_model');
            ...
            $contingut = $this->input->post('comments');
            $tipus = $this->input->post('tipus');

            $idEntrada = $this->blog_model->insertaEntrada($titol, $contingut, $tipus);
            if ($tipus=="2") {
               
            $cont = 1;

               $ext = strtolower(pathinfo($_FILES['foto']['name'], PATHINFO_EXTENSION));
               $nomImatge = sprintf("%05s",   $idEntrada)."_".sprintf("%03s",   $cont);
               $nomImatge = $nomImatge.".".$ext;
                    $this->Redimensionar('images/blog/',$nomImatge,400, 400);
                    $this->blog_model->insertaImatge($idEntrada, $nomImatge,$cont);
            ...
            if ($tipus =="3") {
                $this->blog_model->insertaVideo($idEntrada, $this->input->post('urlVideo'));
            }
    ...
    function blog() {
        ...
        $data = $this->load_page();
        $this->data['message'] = $this->session->flashdata('message');
        $data['main_template']  = 'users/blog';
        $data['title'] = lang('user.opcio3'); 
        $data['description'] = "";
        $this->load->model('blog_model');
        
        $data['result'] = $this->blog_model->getData();  
    ...
    function entrada($idEntrada) {
        ...
        $data = $this->load_page();
        $this->load->model('blog_model');
        $data['result'] = $this->blog_model->getEntrada($idEntrada); 
        $data['imatges'] = $this->blog_model->getImatges($idEntrada); 
        $data['video'] = $this->blog_model->getVideo($idEntrada); 
        $data['main_template']  = 'users/entradaBlog';
        $data['title'] = lang('user.opcio3'); 
        $data['description'] = "";
        $this->load->view('main_template', $data);  
    ...
    function fitxerPla() {
        ...
        $data = $this->load_page();
        $this->data['message'] = $this->session->flashdata('message');
        $data['main_template']  = 'users/fitxer_pla';
        $data['title'] = lang('user.opcio2'); 
        $data['description'] = "";
        $this->load->model('productes_model');
        $data['fitxerPla'] = $this->productes_model->getFitxerPla();  
        $this->load->view('main_template', $data);  
        

    }

    function exporta_fitxer_pla() 
    {


        $this->load->model('productes_model');
        $dades = $this->productes_model->getFitxerPla();


        $arrDades[0] = array_keys($dades[0]);

        $col = 1;
        foreach($dades as $registre) {
            $arrDades[$col] = $registre;
            $col = $col + 1;
        ...
        $this->exporta_xls($arrDades);
    }

    function exporta_xls($dades)
    {
       $this->load->helper('php-excel');   
       $xls = new Excel_XML;
       $xls->addArray ($dades);
       $xls->generateXML ("fitxer");
    }
