...
class Demo_auth_model extends CI_Model {
	...
	function register_account()
	{
		...
		// Set validation errors.
		//if (!in_array($this->data['message'], validation_errors('<p class="error_msg">', '</p>'))) {
			$this->data['message'] = validation_errors('<p class="error_msg">', '</p>');
		//}
	 ...
	 */
	function resend_activation_token($titolMissatge)
	{
		$this->load->library('form_validation');
		...
		{		
			...
			$response = $this->flexi_auth->resend_activation_token($this->input->post('activation_token_identity'), $titolMissatge);
